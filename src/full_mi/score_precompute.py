...
from data import (
    UDDatasetForNEMI,
    collate_fn_for_nemi_training,
    collate_fn_for_nemi_fixz,
)
...
from model import (
    BERTfDivergenceEstimator,
    BERTWassersteinEstimator,
    GPTEnergyEstimator,
    CountBasedEstimator,
    MLMEnergyEstimator,
)
...
argparser_args = [
    ...
    "facebook/opt-350m",
    "--dev_data_file",
    "{}/vinfo_data/en_ewt_extv2-ud-dev-10.conllu".format(working_dir),
    "--dev_sample_dir",
    "{}/cache/opt-samples/bert-base-uncased.facebook_opt-350m".format(working_dir),
    "--dev_sample_fn",
    "en_ewt_extv2-ud-dev-10.conllu.tar.uposmask_qt1.5_SSMH.samples_128",
...
class DataArgumentsForNEMI(DataArguments):
    dev_sample_dir: str = field(
        default="{}/cache/opt-samples/bert-base-uncased.facebook_opt-350m".format(
            working_dir
        )
    )
    dev_sample_fn: str = field(default=None)
    dev_data_file: str = field(
        default="{}/vinfo_data/en_ewt_extv2-ud-dev-10.conllu".format(working_dir)
    )
...
device = torch.device(
    "cuda:{}".format(training_args.gpu_id) if torch.cuda.is_available() else "cpu"
)
...
dev_dataset = UDDatasetForNEMI(
    os.path.join(working_dir, data_args.dev_data_file),
    prop_tokenizer,
    flag_training=False,
)
...
if model_args.energy_model == "clm":
    clm_model = AutoModelForCausalLM.from_pretrained(model_args.clm_model_str).to(
        device
    )
    ...
    d_dict = edict(
        {
            "clm": GPTEnergyEstimator(
                clm_model,
                clm_tokenizer,
                training_args.flag_add_guards_to_clm_tokenizer,
                use_fast_sampler=model_args.flag_use_fast_sampler,
            ),
            **d_dict,
        }
    )
    ...
    raise NotImplementedError(
        f"energy model needs to be either clm or mlm, now is {model_args.energy_model}"
    )
...
with torch.no_grad():
    for _, d in d_dict.items():
        h5f_samples, h5f_scores = d.build_h5(
            data_args.dev_sample_dir, data_args.dev_sample_fn
        )

        dev_dataset.sample_sampled_pairs(
            h5f=h5f_samples, filter_func=filter_dependency_only
        )
        collate_fn = partial(
            collate_fn_for_nemi_fixz, tokenizer=prop_tokenizer, device=device
        )
        train_iter = DataLoader(
            dev_dataset.data, batch_size=1, shuffle=True, collate_fn=collate_fn
        )
        ...
        for inputs, _ in tqdm(train_iter):
            ...
            x, y = inputs.word_idx
            energy_d = d.forward_to_h5(
                inputs.sid, inputs.word_idx, h5f_scores, raws_p, raws_q, device=device
            )
