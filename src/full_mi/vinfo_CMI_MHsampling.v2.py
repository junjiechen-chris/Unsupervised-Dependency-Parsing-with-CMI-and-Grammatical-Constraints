...
class SamplingArguments:
    ...
    energy_temperature: float = field(default=1.0)
...
model_args, data_args, sampling_args = (
...
def pair_filter_base(
    sid, x, upos, flag_exclude_puncts=False, dependency_length_limit=8
):
    sampled_upos = set(
        [
            "PRON",
...
pair_filter = partial(
    pair_filter_base,
    flag_exclude_puncts=sampling_args.flag_exclude_puncts,
    dependency_length_limit=sampling_args.dependency_length_limit,
)

dev_dataset = UDDatasetForCLMSampler(
    os.path.join(working_dir, data_args.dev_data_file), prop_tokenizer
)
dev_dataset.sample(
    pair_filter=pair_filter, flag_strip_accent=sampling_args.flag_strip_accent
)  # nonsyndep_strategy)
...
dev_dataset.sample(
    ...
    left_context=sampling_args.left_context_size,
    right_context=sampling_args.right_context_size,
    max_seqlen=sampling_args.sample_max_seqlen,
)

fn_mask_vocab = (
    get_fullword_mask() if not sampling_args.flag_use_upos_mask else get_upos_mask()
)

dev_dataset.bucketing(
    collate_fn=partial(
        collate_fn_for_clm_sampler,
        device=device,
        tokenizer=prop_tokenizer,
        dataset=dev_dataset,
        mask_vocab_=fn_mask_vocab,
    ),
    limit_tokens=sampling_args.batch_size_tokens,
    tokenizer=prop_tokenizer,
)
...
def build_h5_storage(dataset, target_dir, target_fn, num_samples, mode="r"):
    ...
    if data_args.tar_member is None:
        ...
        dir = os.path.join(
            working_dir,
            target_dir,
            "{}.samples_{}.h5dir".format(target_fn, num_samples),
        )
    ...
    for example in dataset.examples:
        ...
        grp.create_dataset(
            str(sid), shape=(sentence_size, sentence_size, num_samples, 2), dtype=dt
        )
        grp_id.create_dataset(
            str(sid), shape=(sentence_size, sentence_size, num_samples, 2), dtype="i"
        )
        grp_accetpance_rate.create_dataset(
            str(sid), shape=(sentence_size, sentence_size), dtype="f"
        )
...
with torch.no_grad():
    for step_id, (sample, _) in tqdm(
        enumerate(dev_loader), total=dev_dataset._get_chunked_lens()
    ):
        ...
        with torch.autocast(dtype=torch.bfloat16, enabled=False, device_type="cuda"):
            MH_sampling_output = sampling_method(
                inputs,
                sample.token_info,
        ...
        x_samples = vmap_list(
            prop_tokenizer.convert_ids_to_tokens, MH_sampling_output.x_samples.cpu()
        )
        y_samples = vmap_list(
            prop_tokenizer.convert_ids_to_tokens, MH_sampling_output.y_samples.cpu()
        )
        word_samples = [
            [[x, y] for x, y in zip(bx, by)] for bx, by in zip(x_samples, y_samples)
        ]
        id_samples = np.stack(
            [MH_sampling_output.x_samples.cpu(), MH_sampling_output.y_samples.cpu()],
            axis=-1,
        )
        for _sid, _word_idx, _word_samples, _id_samples, _ar in zip(
            sample.sid,
            sample.word_idx,
            word_samples,
            id_samples,
            MH_sampling_output.MH_acceptance_rate.cpu(),
        ):
            h5f["samples"][str(_sid)][_word_idx[0], _word_idx[1]] = _word_samples
            h5f["sample_ids"][str(_sid)][_word_idx[0], _word_idx[1]] = _id_samples
            h5f["acceptance_rate"][str(_sid)][
                _word_idx[0], _word_idx[1]
            ] = _ar  # @MH_sampling_output.MH_acceptance_rate.cpu()
